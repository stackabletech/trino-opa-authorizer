...
----
./trino-cli-395-executable.jar --server http://localhost:8080 --user=admin
----
and for the user `bob`:
----
./trino-cli-395-executable.jar --server http://localhost:8080 --user=bob
----
...
```
-rw-rw-r-- 1 sbernauer sbernauer 33K Sep 15 09:53 /home/sbernauer/stackabletech/trino-opa-authorizer/target/out/trino-opa-authorizer-395-stackable0.1.0.jar
```
...
```
tar -czvf trino-opa-authorizer-395-stackable0.1.0.tar.gz ./trino-opa-authorizer/*
```

Resulting file size should be in the neighborhood of 80 MB:
```
$ ls -lah trino-opa-authorizer-*.gz
-rw-rw-r-- 1 sbernauer sbernauer 86M Sep 15 09:58 trino-opa-authorizer-395-stackable0.1.0.tar.gz
```
